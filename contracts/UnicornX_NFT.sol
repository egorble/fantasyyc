 ...
 * @notice Main ERC-721 NFT contract for UnicornX startup cards
 ...
 */
contract UnicornX_NFT is ERC721, ERC721Enumerable, ERC2981, Ownable2Step, Pausable, ReentrancyGuard {
    ...
    /**
     * @notice Initializes the UnicornX NFT contract
     * @param initialOwner The initial owner of the contract
     */
    constructor(address initialOwner) 
        ERC721("UnicornX Cards", "UNIX") 
    ...
    /**
     * @dev Initialize all 20 startup types with their rarity and multipliers
     * Fundraising details (round, amount, valuation) are stored in backend metadata
     */
    function _initializeStartups() private {
        // Legendary (10x multiplier) - IDs 1-5
        startups[1] = StartupInfo("Openclaw", Rarity.Legendary, 10);
        startups[2] = StartupInfo("Lovable", Rarity.Legendary, 10);              // Series B, $330M at $6.6B valuation
        startups[3] = StartupInfo("Cursor", Rarity.Legendary, 10);               // Series D, $2.3B at $29.3B valuation
        startups[4] = StartupInfo("OpenAI", Rarity.Legendary, 10);               // Series F, $30B
        startups[5] = StartupInfo("Anthropic", Rarity.Legendary, 10);            // Series F, $13B at $183B valuation
        
        // Epic (5x multiplier) - IDs 6-8
        startups[6] = StartupInfo("Browser Use", Rarity.Epic, 5);                // Seed, $17M
        startups[7] = StartupInfo("Dedalus Labs", Rarity.Epic, 5);               // Seed, $11M
        startups[8] = StartupInfo("Autumn", Rarity.Epic, 5);
        
        // Rare (3x multiplier) - IDs 9-13
        startups[9] = StartupInfo("Axiom", Rarity.Rare, 3);
        startups[10] = StartupInfo("Multifactor", Rarity.Rare, 3);               // Seed, $15M
        startups[11] = StartupInfo("Dome", Rarity.Rare, 3);                      // Pre-seed, $500K
        startups[12] = StartupInfo("GrazeMate", Rarity.Rare, 3);                 // Seed, $1.2M
        startups[13] = StartupInfo("Tornyol Systems", Rarity.Rare, 3);           // Seed, $4M
        
        // Common (1x multiplier) - IDs 14-20
        startups[14] = StartupInfo("Pocket", Rarity.Common, 1);                  // Seed, $500K
        startups[15] = StartupInfo("Caretta", Rarity.Common, 1);                 // Seed, $1.3M
        startups[16] = StartupInfo("AxionOrbital Space", Rarity.Common, 1);      // Seed, $500K
        startups[17] = StartupInfo("Freeport Markets", Rarity.Common, 1);        // Pre-seed, $1.1M
        startups[18] = StartupInfo("Ruvo", Rarity.Common, 1);                    // Seed, $4.6M
        startups[19] = StartupInfo("Lightberry", Rarity.Common, 1);              // Seed, $500K
    }
     ...
     *      Example: https://api.unicornx.app/metadata/3421
