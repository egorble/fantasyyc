...
const RightPanel: React.FC<RightPanelProps> = ({ onOpenPack }) => {
  return (
    <aside className="w-80 h-screen fixed right-0 top-0 bg-yc-light-panel dark:bg-yc-dark-panel border-l border-yc-light-border dark:border-yc-dark-border p-6 hidden xl:flex flex-col space-y-6 z-40 overflow-y-auto transition-colors duration-300">
      ...
      {/* Referral */}
      <div className="bg-gradient-to-br from-gray-900 to-black text-white border border-gray-800 dark:border-[#2A2A2A] rounded-xl p-5 relative shadow-lg">
        <div className="flex items-center justify-between mb-3">
            <h3 className="text-white font-bold text-md">Invite Friends</h3>
            <UserPlus className="w-5 h-5 text-yc-orange" />
        </div>
        <p className="text-gray-400 text-xs mb-4 leading-relaxed">
            Get <span className="text-white font-bold">500 XTZ</span> for every friend who joins Season 4.
        </p>
        <div className="flex space-x-2">
            <input type="text" value="YC-JON-8821" readOnly className="bg-black/50 border border-gray-700 dark:border-[#2A2A2A] text-gray-400 text-xs px-3 py-2 rounded flex-1 font-mono focus:outline-none" />
            <button className="bg-gray-700 hover:bg-white hover:text-black text-white text-xs font-bold px-3 py-2 rounded transition-colors">
                Copy
            </button>
        </div>
      </div>
